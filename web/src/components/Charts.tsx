...
export function TrendChart({ data, primary = 'spend', secondary, money = false, height = 280 }: { data: Array<Record<string, unknown>>; primary?: string; secondary?: string; money?: boolean; height?: number }) {
  ...
  return <div style={{ height }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={data} margin={{ top: 10, right: 8, bottom: 0, left: 0 }}>
    ...
    <CartesianGrid stroke="#e9edf4" vertical={false}/><XAxis dataKey="date" tickLine={false} axisLine={false} tick={{ fill: '#7b8494', fontSize: 11 }} minTickGap={28}/><YAxis tickLine={false} axisLine={false} tickFormatter={(v) => money ? formatIdr(v) : formatCompact(v)} tick={{ fill: '#7b8494', fontSize: 11 }} width={62}/>
    <Tooltip contentStyle={tooltipStyle} formatter={(v) => money ? formatIdr(Number(v)) : formatCompact(Number(v))}/><Legend iconType="circle" iconSize={7}/>
...
export function HorizontalBars({ data, dataKey = 'value', nameKey = 'label', money = false, height = 260 }: { data: Array<Record<string, unknown>>; dataKey?: string; nameKey?: string; money?: boolean; height?: number }) {
  ...
  return <div style={{ height }}><ResponsiveContainer width="100%" height="100%"><BarChart data={data} layout="vertical" margin={{ left: 12, right: 26 }}>
    <CartesianGrid stroke="#e9edf4" horizontal={false}/><XAxis type="number" hide/><YAxis type="category" dataKey={nameKey} width={100} tickLine={false} axisLine={false} tick={{ fill: '#596273', fontSize: 11 }}/><Tooltip contentStyle={tooltipStyle} formatter={(v) => money ? formatIdr(Number(v)) : formatCompact(Number(v))}/><Bar dataKey={dataKey} radius={[0, 7, 7, 0]} barSize={14}>{data.map((_, i) => <Cell key={i} fill={colors[i % colors.length]}/>)}</Bar>
