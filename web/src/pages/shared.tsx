...
export const emptyPaid = (accountId = '—'): PaidBlock => ({ source:'WAC reporting warehouse', accountId, currency:'IDR', available:false, freshness:null, qaFlags:[], kpis:['spend','impressions','reach','clicks','ctr','cpm'].map(metric => ({metric,value:null,comparison:null,delta:null})), series:[], campaigns:[], ads:[] });
...
export function PaidMetrics({ data, platform }: { data: PaidBlock; platform: string }) {
  return <section className="content-section"><SectionHeading kicker="03 · Performance media" title={`${platform} paid media`} description="Normalized daily facts from the WAC reporting warehouse. Account totals and campaign entities use the same selected window." action={<div className="source-badges"><StatusPill status={data.available ? 'live' : 'unavailable'} label={data.available ? 'data available' : 'awaiting backfill'}/>{data.freshness && <StatusPill status={data.freshness.states.includes('partial') ? 'partial' : 'live'} label={data.freshness.latestReportDate}/>}</div>}/>
    ...
    <div className="kpi-grid six">{['spend','impressions','reach','clicks','ctr','cpm'].map((metric,index) => { const item=kpi(data,metric); return <KpiCard key={metric} label={titleCase(metric)} metric={metric} value={item?.value} delta={item?.delta} accent={['indigo','blue','teal','orange','rose','violet'][index]}/>; })}</div>
    <div className="split-grid wide-left"><Card><div className="card-head"><div><strong>Spend × delivery</strong><span>{data.freshness ? `Facts through ${data.freshness.latestReportDate}` : 'Awaiting the first successful ingestion'}</span></div><Clock3 size={18}/></div><TrendChart data={data.series} primary="spend" secondary="impressions" money/></Card><Card><div className="card-head"><div><strong>Campaign breakdown</strong><span>Funnel tags inferred from entity naming</span></div><DatabaseZap size={18}/></div><MetricTable rows={data.campaigns} emptyMessage="Campaign-level facts are unavailable for this window."/></Card></div>
